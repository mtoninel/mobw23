...
- _Learn about the `DESeq2` package_
- _Explore different normalization methods_
- _Normalize the data with functions provided by the `DESeq2` package_
...
```{r, echo=FALSE}
sizeFactors(dds) %>% as.data.frame() %>% kbl() %>% kable_styling()
```
...
```{r, echo=FALSE}
head(assay(rld), 10) %>% kbl() %>% kable_styling()
```
...
> 💡
> **It is clear how now genes with high mean expression values (on the right) are comparable in terms of standard deviation to genes with lower mean expression values (on the left).**
...
> 💡
> **What type of assessment would you make about the consistency of the samples across these top 200 genes? Do they cluster (a synonym for _similar_) based on the donor or on the biological condition of our interest?**
